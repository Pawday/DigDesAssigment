...
(
    SELECT
    (SELECT count("ID") FROM "DEPARTMENT") AS "DEPARTMENTS_AMOUNT",
    (SELECT 0) AS "DEPARTMENT_OFFSET",
    ...
    SELECT
        "DEPARTMENTS_AMOUNT",
        "DEPARTMENT_OFFSET"+1 AS "DEPARTMENT_INDEX",
        (SELECT "ID" FROM "DEPARTMENT" LIMIT 1 OFFSET "DEPARTMENT_OFFSET" + 1) AS "DEP_ID",
        (SELECT sum("SALARY") FROM "EMPLOYEE" WHERE "DEPARTMENT_ID" IN ((SELECT "ID" FROM "DEPARTMENT" LIMIT 1 OFFSET ("DEPARTMENT_OFFSET" + 1)))) AS "DEP_SALARY_SUM"
    FROM R
    WHERE "DEPARTMENT_OFFSET" < "DEPARTMENTS_AMOUNT" - 1
)
